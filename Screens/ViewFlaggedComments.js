...
export default function ViewFlaggedComments({navigation}) {
    const [comment, setComment] = useState(null);
    const nav = useNavigation();
    const navtoFCom = (item) => {
    ...
    return (
        <View style={{ backgroundColor: "#000", width: Dimensions.get("window").width, height: Dimensions.get("window").height }}>
                ...
                >
                    <View>
                        <Text style={{ padding: 20, fontWeight: "bold", fontSize: 25 }}>View Flagged Comments</Text>
                    </View>
                        ...
                        <View key={item._id}>
                            <Pressable style={{ backgroundColor: "#F8F8F8", borderRadius: 8, padding: 10, alignItems: 'flex-start', justifyContent: 'space-between', margin: 14 }}>
                                <View style={{ backgroundColor: "#F8F8F8" }}>
                                    ...
                                    <Pressable onPress={navtoFCom(item)} style={styles.pressable}>
