...
import carrotPlant from "../images/carrot_2.png";
...
export default function ChoseLocation(props, { navigation }) {
  ...
  const uploadImage = async () => {
    ...
    if (selectedImage != null) {
      ...
      const childPath =
        "posts/" + user + "/" + Math.random().toString(36) + ".jpeg";
  ...
  const savePostData = async (photos) => {
    addDoc(collection(db, "Posts"), {
      ...
      .catch((error) => {
        ...
        showMessage(
          "Error!",
          "There was an error posting the picture. Please try again!"
        );
      });
  };
  ...
  return (
    <SafeAreaView style={{ marginTop: "6%", flex: 1 }}>
      ...
      <TouchableOpacity
        ...
        onPress={uploadImage}
      >
        <Image style={styles.image} source={carrotPlant} resizeMode="contain" />
      </TouchableOpacity>
    </SafeAreaView>
  );
}
